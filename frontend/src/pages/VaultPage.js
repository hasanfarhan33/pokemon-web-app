...
const VaultPage = () => {
    ...
    const handleRemove = async (pokemonName) => {
        const userId = user._id
        
        try {
            const response = await fetch(`/api/pokeapi/favorites/remove/${userId}/${pokemonName}`, {
                method: "DELETE", 
                headers: {
                    'Content-Type': "application/json",
                    Authorization: `Bearer ${user.token}`
            ...
            if (!response.ok) {
                throw new Error(data.message || "Failed to remove Pokemon")
            }

            setFavorites((prevFavorites) => prevFavorites.filter(name => name !== pokemonName)); 
            setFavoriteDetails((prevDetails) => prevDetails.filter(pokemon => pokemon.name !== pokemonName))
